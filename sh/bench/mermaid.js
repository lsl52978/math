...
const ROOT = join(import.meta.dirname, "..", ".."),
  ...
  bar = (label, val, max, y, color, fmt) => {
    const w = Math.max(8, (val / max) * 230);
    return (
      '<text x="24" y="' +
      (y + 18) +
      '">' +
  ...
  chart = (title, rows, fmt) => {
    const max = Math.max(...rows.map((r) => r[1]));
    return (
      '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 560 160"><style>text{font:14px -apple-system,BlinkMacSystemFont,Segoe UI,sans-serif;fill:#0f172a}.title{font-weight:700;font-size:18px}.value{text-anchor:end}</style><rect width="560" height="160" rx="18" fill="#f8fafc"/><text class="title" x="24" y="34">' +
      title +
      "</text>" +
      rows.map((r, i) => bar(r[0], r[1], max, 58 + i * 42, r[2], fmt)).join("") +
      "</svg>"
    );
  },
  speed = () => {
  ...
  run = async () => {
    ...
    writeFileSync(
      ...
      chart(
        ...
        (val) => val.toLocaleString("en-US") + " svg/s",
      ),
    );
